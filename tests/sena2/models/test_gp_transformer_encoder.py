...
from sena2.models.gp_transformer_encoder import (
    AttentionLayer,
    GeneEmbeddingLayer,
...
class TestAttentionLayer:
    ...
    def test_scaled_dot_product_attention(self):
        ...
        attention_layer = AttentionLayer(self.d_embed, self.n_heads)
    ...
    def test_scaled_dot_product_attention_static_mask(self):
        Q = torch.rand((self.batch_size, self.n_heads, self.q_len, self.d_head))  # noqa: N806
        K = torch.rand((self.batch_size, self.n_heads, self.k_len, self.d_head))  # noqa: N806
        V = torch.rand((self.batch_size, self.n_heads, self.k_len, self.d_head))  # noqa: N806
        W = torch.randint(2, (self.batch_size, self.q_len, self.k_len))  # noqa: N806

        attention_layer = AttentionLayer(self.d_embed, self.n_heads, mask_type="static")
    ...
    def test_scaled_dot_product_attention_dynamic_mask(self):
        Q = torch.rand((self.batch_size, self.n_heads, self.q_len, self.d_head))  # noqa: N806
        K = torch.rand((self.batch_size, self.n_heads, self.k_len, self.d_head))  # noqa: N806
        V = torch.rand((self.batch_size, self.n_heads, self.k_len, self.d_head))  # noqa: N806
        ...
        attention_layer = AttentionLayer(
            self.d_embed, self.n_heads, mask_type="dynamic"
        )
        output, attn_weights = attention_layer.scaled_dot_product_attention(Q, K, V, W)
...
class TestGenePathwayTransformerEncoder:
    ...
    def test_model_parameters(self):
        ...
        expected_parameters = {
            "W": torch.Size([self.n_genes, self.n_pathways]),
    ...
    def test_forward_static_mask(self):
        gene_indices = torch.randint(self.n_genes, (self.batch_size, self.genes_len))
        expression_values = torch.rand((self.batch_size, self.genes_len))
        pathway_indices = torch.randint(
            self.n_pathways, (self.batch_size, self.n_pathways)
        )
        W_init = torch.randint(2, (self.genes_len, self.n_pathways))  # noqa: N806
    ...
    def test_forward_dynamic_mask(self):
        gene_indices = torch.randint(self.n_genes, (self.batch_size, self.genes_len))
        expression_values = torch.rand((self.batch_size, self.genes_len))
        pathway_indices = torch.randint(
            self.n_pathways, (self.batch_size, self.n_pathways)
    ...
    def test_forward_static_mask_change(self):
        for genes_len in range(0, self.n_genes, 10):
            gene_indices = torch.randint(self.n_genes, (self.batch_size, genes_len))
            expression_values = torch.rand((self.batch_size, genes_len))
            pathway_indices = torch.randint(
                self.n_pathways, (self.batch_size, self.n_pathways)
            )
            W_init = torch.randint(2, (genes_len, self.n_pathways))  # noqa: N806
    ...
    def test_forward_dynamic_mask_change(self):
        for genes_len in range(0, self.n_genes, 10):
            gene_indices = torch.randint(self.n_genes, (self.batch_size, genes_len))
            expression_values = torch.rand((self.batch_size, genes_len))
            pathway_indices = torch.randint(
