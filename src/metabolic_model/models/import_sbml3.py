"""Import SBML3 models into a MetabolicModel object.
...
def association_from_xml(xml_node):
    ...
    if isinstance(xml_node, libsbml.FbcOr):
        ...
        association.children = [
            association_from_xml(xml_node=x) for x in xml_node.getListOfAssociations()
        ]
...
def gene_from_xml(xml_node):
    """Build the gene from the xml node."""
    gene = Gene()
