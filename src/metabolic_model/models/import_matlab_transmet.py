...
#     Species,
# )
# from .metabolic_model_transmet import (
#     MetabolicModelTransmet,
#     Subsystem,
# )
...
#     groups = zip(rxns, rxn_names, lbs, ubs, subsystems, rules)
#     reactions = []
#     subsystems = {}
...
#             subsystems[subsystem] = Subsystem(name=subsystem)

#         subsystems[subsystem].add_reaction(reaction)
...
#         rxn_meta = pd.read_csv(fname, sep="\t", index_col=0, quoting=3)

#         for i, reaction in enumerate(reactions):
#             reaction.meta = _fix_dtypes(rxn_meta.loc[reaction.id].to_dict())
...
#         met_names = json.load(fin)
#     with open(os.path.join(model_dir, "model.metFormulas.json")) as fin:
#         met_formulas = json.load(fin)

#     species = []
#     compartment_re = re.compile(r"\[(.+)\]")
#     for met_id, name, formula in zip(met_ids, met_names, met_formulas):
#         met = Species()
#         met.id = met_id
#         met.name = name
#         met.formula = formula
...
#     if os.path.exists(fname):
#         # quoting=3 setting ignores " in file
#         met_meta = pd.read_csv(fname, sep="\t", index_col=0, quoting=3)

#         for i, met in enumerate(species):
#             met.meta = _fix_dtypes(met_meta.loc[met.id].to_dict())
...
#     model = MetabolicModelTransmet(name)
...
#     """Use to change numpy.int64 and numpy.float64 to python types.
