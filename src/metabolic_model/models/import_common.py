"""Common functions for importing metabolic models.
...
def clean_reactions(model):
    """Fix reactions so that species are not listed as both reactants and products."""
...
def limit_maximum_flux(model, new_limit):
    """Clips flux limits so that the max value is equal to new_limit."""
    if new_limit < 0:
