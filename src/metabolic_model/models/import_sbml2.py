"""Import SBML models into a MetabolicModel object.
...
def load(model_name, sbml_document):
    """Load the metabolic model from an SBML document, returning a Model object."""
...
def reaction_from_xml(xml_node):
    ...
    if not found_lb or not found_ub:
        raise Exception(
            "Error, could not find upper and lower bounds for reaction " + reaction.id
        )
    ...
    gene_association_str = (
        GENE_ASSOCIATION_RE.search(xml_node.getNotesString()).group(1).strip()
    )
...
def association_from_xml(rule_str):
    """Build the association from the xml node."""
...
def compartment_from_xml(xml_node):
    """Build the compartment from the xml node."""
...
def species_from_xml(xml_node):
    """Build the species from the xml node."""
    species = Species()
