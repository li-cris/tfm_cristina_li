...
from .metabolic_model import Association, Gene

this_directory = os.path.dirname(os.path.abspath(__file__))


def detect_type(model):
    """Detect the type of gene symbols in its genes' name field in the model.
...
def resolve_genes(model):
    """Ensure the gene names in a model correspond with hgnc gene symbols.
    ...
    hgnc_file = os.path.join("Resources", "Genes", "HGNC.json.gz")
...
def load_mgi():
    """Load the ortho2human, ortho2mouse dictionaries.
    ...
    mgi_file = os.path.join("Resources", "Genes", "HOM_MouseHumanSequence.rpt.gz")
...
def convert_species(model, target_species):
    """Convert the gene symbols in a model to another species."""
