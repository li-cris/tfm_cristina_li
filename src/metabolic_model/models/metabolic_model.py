...
class MetabolicModel(object):
    ...
    def getReactions(self):
        """Returns a list of reaction id's in the MetabolicModelCompass"""
    ...
    def getReactionExpression(
        self, expression, and_function="min", or_function=sum_wo_nan
    ):
    ...
    def _calc_max_flux(self):
        ...
        for reaction in self.reactions.values():
            max_flux = max(
                abs(reaction.lower_bound), abs(reaction.upper_bound), max_flux
            )
...
class Reaction(object):
    ...
    def eval_expression(self, expression, and_function, or_function):
        ...
        if self.gene_associations is None:
            ...
            return self.gene_associations.eval_expression(
...
def _print_node(node, expression=None, indent=0):
    ...
    if node.type == "gene":
        lines.append(
            " ".join(
                [" " * indent, node.gene.id, node.gene.name, str(node.gene.alt_symbols)]
            )
        )
