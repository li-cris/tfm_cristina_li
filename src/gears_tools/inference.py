...
class MetricEvaluator:
    """
    Class to evaluate predicted results in GEARS.
    ...
    """
    def __init__(self, adata: AnnData, model_name: str, results_savedir: str,
                 pool_size: int = 250, seed: int = 42, top_deg: int = 20):
        self.adata = adata
        self.model_name = model_name
        self.results_savedir = results_savedir
        self.pool_size = pool_size
        self.seed = seed
        self.top_deg = top_deg

    def evaluate_double(self) -> None:
        """Evaluate the predicted GEPs of double perturbations."""
        # Load predicted GEPs.
        df = pd.read_csv(
            filepath_or_buffer=os.path.join(self.results_savedir, f"{self.model_name}_double.csv")
        )

        # Make results file path.
        results_file_path = os.path.join(
            self.results_savedir, f"{self.model_name}_double_metrics.csv"
        )

        with open(file=results_file_path, mode="w") as f:
            print(
                f"double,mmd_true_vs_ctrl,mmd_true_vs_pred,mse_true_vs_ctrl,mse_true_vs_pred,kld_true_vs_ctrl,kld_true_vs_pred,PearsonTop{self.top_deg}_true_vs_ctrl,Pearson_pval_true_vs_ctrl,PearsonTop{self.top_deg}_true_vs_pred,Pearson_pval_true_vs_pred",
                file=f,
            )
            ...
            for i, double in enumerate(df["double"]):
                ...
                true_geps = self.adata[self.adata.obs["condition"] == double]
                ...
                topdeg_idx = np.argsort(abs(true_deg))[-self.top_deg:]
                ...
                print(f"Pearson Top {self.top_deg} DEG (true vs. control): {pearson_true_vs_ctrl.statistic:.6f} | p-value: {pearson_true_vs_ctrl.pvalue:.6f}")
                print(f"Pearson Top {self.top_deg} DEG (true vs. predicted): {pearson_true_vs_pred.statistic:.6f} | p-value: {pearson_true_vs_pred.pvalue:.6f}")
    ...
    def evaluate_single(self) -> None:
        ...
        # Load predicted GEPs.
        df = pd.read_csv(
            filepath_or_buffer=os.path.join(self.results_savedir, f"{self.model_name}_single.csv")
        )

        # Make results file path.
        results_file_path = os.path.join(
            self.results_savedir, f"{self.model_name}_single_metrics.csv"
        )

        with open(file=results_file_path, mode="w") as f:
            print(
                f"single,split1,split2,mmd_true_vs_ctrl,mmd_true_vs_pred,mse_true_vs_ctrl,mse_true_vs_pred,kld_true_vs_ctrl,kld_true_vs_pred,PearsonTop{self.top_deg}_true_vs_ctrl,Pearson_pval_true_vs_ctrl,PearsonTop{self.top_deg}_true_vs_pred,Pearson_pval_true_vs_pred",
                file=f,
            )
